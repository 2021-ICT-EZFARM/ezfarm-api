...
public class RemoteAcceptanceTest extends CommonAcceptanceTest {
  ...
  void updateRemoteAndCreateHistory() throws Exception {
      ...
      Assertions.assertAll(
          ...
          () -> assertThat(remoteHistories.get(0).getValue()).isEqualTo("Water 설정을 OFF에서 ON(으)로 변경하였습니다"),
          () -> assertThat(remoteHistories.get(1).getValue()).isEqualTo("Temperature 설정을 0.0에서 37.5(으)로 변경하였습니다"),
          () -> assertThat(remoteHistories.get(2).getValue()).isEqualTo("Co2 설정을 OFF에서 ON(으)로 변경하였습니다")
      );
