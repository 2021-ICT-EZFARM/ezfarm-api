  ...
  private void facilityAveraging(Farm bestFarm, List<Facility> facilities) {
    ...
    for (Facility facility : facilities) {
      ...
      if (day != facility.getMeasureDate().get(weekFields.dayOfWeek())) {
        ...
        if (bestFarm.getId() == 20) {
          Farm farm = farmRepository.findById(1L).get();
          facilityDayAvgRepository.save(
              FacilityDayAvg.builder().farm(farm).facilityAvg(tmpFacilityDay)
                  .measureDate(facility.getMeasureDate()).build());
        } else if (bestFarm.getId() == 21) {
          Farm farm = farmRepository.findById(2L).get();
          facilityDayAvgRepository.save(
              FacilityDayAvg.builder().farm(farm).facilityAvg(tmpFacilityDay)
                  .measureDate(facility.getMeasureDate()).build());
        }
      ...
      if (week != facility.getMeasureDate().get(weekFields.weekOfMonth())) {
        ...
        if (bestFarm.getId() == 20) {
          Farm farm = farmRepository.findById(1L).get();
          facilityWeekAvgRepository
              .save(FacilityWeekAvg.builder().farm(farm).facilityAvg(tmpFacilityWeek)
                  .measureDate(facility.getMeasureDate()).build());
        } else if (bestFarm.getId() == 21) {
          Farm farm = farmRepository.findById(2L).get();
          facilityWeekAvgRepository
      ...
      if (month != facility.getMeasureDate().getMonthValue()) {
        ...
        if (bestFarm.getId() == 20) {
          Farm farm = farmRepository.findById(1L).get();
          facilityMonthAvgRepository
              .save(FacilityMonthAvg.builder().farm(farm).facilityAvg(tmpFacilityMonth)
                  .measureDate(facility.getMeasureDate()).build());
        } else if (bestFarm.getId() == 21) {
          Farm farm = farmRepository.findById(2L).get();
          facilityMonthAvgRepository
